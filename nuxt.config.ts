...
export default defineNuxtConfig({
  ...
  // Project-aware flat config, regenerated by `nuxt prepare` into
  // .nuxt/eslint.config.mjs (root eslint.config.mjs wraps it via withNuxt).
  // stylistic is OFF on purpose: Prettier + prettier-plugin-tailwindcss own
  // formatting, so the linter stays behavior-only and can't fight the formatter.
  eslint: {
    config: {
      stylistic: false,
    },
